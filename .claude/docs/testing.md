...
Two tiers, each with a dedicated subagent.
...
## Running everything
- Slash command: `/iterate` runs both in parallel via subagents and
  then delegates to the optimizer.
- Cheap state check: `/snapshot` runs visual-qa only.

## Smoke (legacy)
`e2e/smoke.spec.ts` is the `@playwright/test` smoke. It is
...
Both agents produce JSON reports ≤3KB each. The optimizer agent
consumes all three and picks **one** fix. One iteration = one commit.

## TDD discipline
The agents above are **runners** — read-only by contract. The
