...
+ Tailwind** static SPA at the repo root, hosted on **Cloudflare Pages** at
https://ghulamsiddiq.com. No backend: booking goes through WhatsApp/tel:
links; the booking-feedback POST currently has no server and fails silently.
Deploy with `./infra/deploy.sh`.
...
- **Design tokens**: [.claude/docs/design-tokens.md](.claude/docs/design-tokens.md) — palette, type, radii, shadows. Source of truth alongside `tailwind.config.ts` + `src/design-system/tokens.css`.
- **Architecture**: [.claude/docs/architecture.md](.claude/docs/architecture.md) — routes, components, content files, deploy.
...
- **Testing**: [.claude/docs/testing.md](.claude/docs/testing.md) — visual-qa + ux-flow scripts.
...
## Agents & commands
- `/iterate` — one 2h product-cycle loop: visual-qa → ux-flow → optimizer → commit.
...
3. **Content is data-driven.** User-visible strings live in `src/content/*.ts`. Do not hardcode copy in components.
